...
from battlezone.protocol import (
    ...
    NETWORK_RENDER_DELAY as DEFAULT_NETWORK_RENDER_DELAY,
    NETWORK_SMOOTHING_RATE,
)

from battlezone.controllers import RemoteTankController, TankCommand, TankController
from battlezone.network import UdpTankInputBridge
...
NETWORK_CLIENT_LOW_RENDER_SIZE = env_size("BATTLEZONE_NET_CLIENT_LOW_RENDER_SIZE", (960, 540))
NETWORK_SERVER_LOW_RENDER = env_bool("BATTLEZONE_NET_SERVER_LOW_RENDER", True)
NETWORK_SERVER_LOW_RENDER_SIZE = env_size("BATTLEZONE_NET_SERVER_LOW_RENDER_SIZE", (720, 405))
NETWORK_RENDER_DELAY = env_float("BATTLEZONE_NET_RENDER_DELAY", DEFAULT_NETWORK_RENDER_DELAY)
...
class MyApp(ShowBase):
    ...
    def capture_network_human_input(self, bridge):
        ...
        fire = fire_down and not bridge.last_fire_down
        bridge.last_fire_down = fire_down
    ...
    def setup_network_controller_prototype(self):
        ...
        try:
            self.network_bridge = UdpTankInputBridge(
                ...
                NETWORK_PORT,
                NETWORK_TANK_ID,
                tank_ids=network_tank_ids(),
                claimable_tank_ids=CLAIMABLE_TANK_IDS,
                command_type=TankCommand,
                human_input_reader=self.capture_network_human_input
            )
    ...
    def tank_contacts_other_tank(self, tank_id, pos, body_radius, tank_states=None, extra_padding=0.0):
        pos = Point3(pos)
        for _other_tank_id, other_pos, other_radius in self.other_tank_collision_bodies(tank_id, tank_states):
            min_dist = body_radius + other_radius + PLAYER_TANK_COLLISION_CONTACT_PADDING + extra_padding
            dx = pos[0] - other_pos[0]
            dy = pos[1] - other_pos[1]
    ...
    def resolve_tank_collision_swept_position(self, tank_id, start_pos, target_pos, body_radius, tank_states=None):
        start = Point3(start_pos)
        ...
        for _other_tank_id, other_pos, other_radius in self.other_tank_collision_bodies(tank_id, tank_states):
            min_dist = body_radius + other_radius + PLAYER_TANK_COLLISION_CONTACT_PADDING
            sx = start[0] - other_pos[0]
            sy = start[1] - other_pos[1]
            a = vx * vx + vy * vy
            if a < 0.0001:
                continue
            ...
            entry_t = (-b - math.sqrt(discriminant)) / (2.0 * a)
    ...
    def apply_player_tank_command(self, command, dt):
        ...
        if command.throttle:
            ...
            start_pos = Point3(self.camera.getPos(render))
            next_pos = self.resolve_obstacle_position(
                start_pos + step,
                body_radius
            )
            next_pos, tank_collision = self.resolve_tank_collision_swept_position(
                "0",
                start_pos,
                next_pos,
                body_radius
    ...
    def apply_autonomous_player_tank_command(self, command, dt):
        ...
        avoided_world, tank_collision = self.resolve_tank_collision_swept_position(
            "0",
            current_world,
            avoided_world,
            body_radius
    ...
    def apply_direct_tank_command(self, t, command, dt):
        ...
        if command.throttle:
            ...
            start_pos = Point3(tank_np.getPos(render))
            avoided_world = self.resolve_obstacle_position(
                start_pos + step,
                body_radius
            )
            avoided_world, _tank_collision = self.resolve_tank_collision_swept_position(
                t,
                start_pos,
                avoided_world,
                body_radius
