...
Joystick input is enabled by default on clients. Use `BATTLEZONE_JOYSTICK_ENABLED=0` to force keyboard-only input, or tune stick drift with `BATTLEZONE_JOYSTICK_DEADZONE`, for example `0.22`.
